...
toolbox.register("individual", tools.initCycle, creator.Individual,
                 (toolbox.num_particles,
                  toolbox.motion_noise,
                  toolbox.patch_size), n=1)
...
def evaluate(ind):
    ...
    start = time.time()
    # This call spins up your tracker → on GPU
    mota, idsw, fps = run_tracking_evaluation(
        VIDEO_PATH,
        num_particles, motion_noise, patch_size,
        max_frames=150
        
    )
    elapsed = time.time() - start

    print(f"✅ Eval: MOTA={mota:.3f}, IDSW={idsw}, FPS={fps:.2f}, Time={elapsed:.1f}s")
...
def run_qpso(pop_size=20, max_gen=10, alpha=0.75):
    ...
    for gen in range(max_gen):
        print(f"\n🔄 GEN {gen} — BEST so far: MOTA={gbest_fit[0]:.3f}, FPS={gbest_fit[2]:.2f}")
        log_lines.append(f"GEN {gen} ⇒ BEST: MOTA={gbest_fit[0]:.3f}, FPS={gbest_fit[2]:.2f}")

        mbest = np.mean(pbest, axis=0)

        for i, ind in enumerate(pop):
            # QPSO update per-dimension
            for d in range(3):
                u = random.random()
                sign = -1 if random.random() < 0.5 else 1
                ind[d] = (
                    pbest[i][d]
                    + sign * alpha * abs(mbest[d] - pbest[i][d]) * np.log(1.0 / u)
                )
            ...
            # occasional jitter for exploration
    ...
    print(f"\n🏆 FINAL BEST: {gbest}, Fitness: {gbest_fit}")
